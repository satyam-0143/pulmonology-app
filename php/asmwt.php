...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST["pid"])) {
        ...
        $sql1 = mysqli_query($conn, "SELECT pbp FROM asmwt where pid='$id' ");
        $sql2 = mysqli_query($conn, "SELECT phr FROM asmwt where  pid ='$id' ");
        $sql3 = mysqli_query($conn, "SELECT pso2 FROM asmwt where pid='$id' ");
        $sql4 = mysqli_query($conn, "SELECT pdys FROM asmwt where  pid ='$id' ");
        $sql5 = mysqli_query($conn, "SELECT pfat FROM asmwt where pid='$id' ");
        $sql6 = mysqli_query($conn, "SELECT ebp FROM asmwt where  pid ='$id' ");
        $sql7 = mysqli_query($conn, "SELECT ehr FROM asmwt where pid='$id' ");
        $sql8 = mysqli_query($conn, "SELECT eso2 FROM asmwt where  pid ='$id' ");
        $sql9 = mysqli_query($conn, "SELECT edys FROM asmwt where pid='$id' ");
        $sql10 = mysqli_query($conn, "SELECT efat FROM asmwt where  pid ='$id' ");
        $sql11= mysqli_query($conn, "SELECT t1 FROM asmwt where pid='$id' ");
        $sql12= mysqli_query($conn, "SELECT t2 FROM asmwt where  pid ='$id' ");
