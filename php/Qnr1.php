...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST["score"]) && isset($_POST["pid"])) {
        ...
        $sql1 = mysqli_query($conn, "SELECT 1Qnr1 FROM patient_info where pid='$id' ");
        $sql2 = mysqli_query($conn, "SELECT 2Qnr2 FROM patient_info where  pid ='$id' ");
        $row = mysqli_fetch_assoc($sql1);
        $row1= mysqli_fetch_assoc($sql2);

        // Access the value of 1Qnr1 column from the fetched row
        $valueFromSQL1 = $row['1Qnr1'];
        $valueFromSQL2 = $row1['2Qnr2'];
        ...
        if (  $valueFromSQL1 === "") {
            ...
            mysqli_query($conn, "update patient_info set 1Qnr1='$score' where pid='$id'  ");
        } elseif ($valueFromSQL2 === "") {
            // If sql2 result is empty, perform another insert or update
            mysqli_query($conn, "update patient_info set 2Qnr2='$score' where pid='$id'  ");
        }
